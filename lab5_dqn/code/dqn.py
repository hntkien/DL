...
def init_weights(m):
    ...
    Args:
        m (nn.Module): Module to initialize in-place.
    """
...
class DQN(nn.Module):
    ...
    def __init__(self, num_actions, input_dim=None):
        ...
        if not self.is_image:
            ...
            # Task 2/3: Classic Atari CNN (Mnih 2015)
            self.network = nn.Sequential(
    ...
    def forward(self, x):
        # Normalize pixel intensities to [0,1] for CNN mode
        if self.is_image:
            x = x / 255.0
...
class PrioritizedReplayBuffer:
    """Prioritized replay (Schaul et al., 2016). To be filled in Task 3."""
...
class DQNAgent:
    """Vanilla DQN agent supporting both vector and image observations.

    Args:
        env_name (str): Gymnasium environment id (e.g., 'CartPole-v1', 'ALE/Pong-v5').
        args (argparse.Namespace): Hyperparameters / paths.
    """
    ...
    def run(self, episodes=1000):
        """Outer training loop: collect transitions, train, evaluate, checkpoint."""
    ...
    def evaluate(self):
        """Greedy roll-out for ``num_eval_episodes`` and return mean episodic reward."""
        total = 0.0
    ...
    def train(self):
        """One gradient update on a uniformly-sampled mini-batch."""
        if len(self.memory) < max(self.replay_start_size, self.batch_size):
            return

        # Decay epsilon (once per training update)
        if self.epsilon > self.epsilon_min:
            self.epsilon *= self.epsilon_decay
        self.train_count += 1

        # Sample a mini-batch of (s, a, r, s', done)
...
def _build_parser(parents=()):
    """Construct the full argparse parser. Used for both stages of parsing.

    All defaults can be overridden by (a) a YAML config via ``--config`` and
    (b) explicit CLI flags. CLI takes precedence over YAML.
    """
    ...
    parser.add_argument(
        "--grad_clip", type=float, default=10.0,
        help="Max L2 norm for gradient clipping; <=0 disables.",
    )
    # Exploration
    parser.add_argument("--epsilon_start", type=float, default=1.0)
    parser.add_argument(
        "--epsilon_decay", type=float, default=0.9995,
...
def parse_args():
    ...
    Returns:
        argparse.Namespace: Final, merged config.
    """
    # Stage 1: capture --config without forcing it
    ...
    # Stage 2: full parser, YAML values become new defaults, CLI overrides them
